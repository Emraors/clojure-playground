...
(defn parse-input
  [input]
  (map (comp parse-int #(str/trim %)) (str/split input #"\s*,\s*")))

(defn constant-cost [final-pos initial-pos] (abs (- final-pos initial-pos)))
...
(defn state-cost
  [eval-fun positions aiming-position]
  (->> positions
...
(assert (= 37 (eval-solution constant-cost example-input)))

(assert (= 356922 (eval-solution constant-cost input)))

(assert (= 206 (eval-solution linear-cost example-input)))

(assert (= 100347031 (eval-solution linear-cost input)))
