...
(defn parse-numbers [input] (map parse-int (str/split input #",")))

(defn parse-table
  [input]
  (let [line->ints (comp (map str/trim)
                         (map #(str/split % #"\s+"))
                         (map #(map parse-int %)))]
    (into '() line->ints (parse-lines input))))

(defn extract-number [state] (first (get state :numbers)))

(defn last-extracted-number [state] (first (get state :extracted-numbers)))

(defn playable-tables [state] (get-in state [:tables :non-winning-tables]))

(defn winning-tables [state] (get-in state [:tables :winning-tables]))
...
(defn mark-table
  [number table]
  (map (fn [row] (map (fn [elem] (if (= elem number) marker elem)) row)) table))

(defn bingo?
  [table]
  (let [rows (apply map list table)
        winning-condition (fn [el] (boolean (some #(every? #{marker} %) el)))]
    (or (winning-condition table) (winning-condition rows))))
...
(defn next-state
  ...
  (let [current-number (extract-number state)]
    (-> state
        ...
        (update :tables (partial mark-and-check-tables current-number)))))

(defn first-winning? [state] (not (empty? (winning-tables state))))

(defn last-winning? [state] (empty? (playable-tables state)))
...
(assert (= 4512 (get-solution example-input first-winning?)))
(assert (= 60368 (get-solution input first-winning?)))

(assert (= 1924 (get-solution example-input last-winning?)))

(assert (= 17435 (get-solution input last-winning?)))
