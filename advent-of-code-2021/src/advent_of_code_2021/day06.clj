...
;; my original solution was too slow, I took the idea from:
;; https://github.com/abyala/advent-2021-clojure/blob/main/docs/day06.md
...
(defn parse-input
  [input]
  (merge empty-state
         (frequencies (map (comp parse-int #(str/trim %))
                        (str/split input #"\s*,\s*")))))

(defn update-timer [timer] (if (zero? timer) reset-timer (dec timer)))

(defn next-generation
  [state]
  (let [deliveries (state delivery-timer)]
    (->
      (reduce-kv (fn [m k v] (update m (update-timer k) + v)) empty-state state)
