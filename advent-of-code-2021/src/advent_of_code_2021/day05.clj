...
(defn parse-input [input]
  (map parse-hydrothermal (parse-lines input)))

;; a segment is just a list of points, where a point is a map of the form
;; {:x x_0 :y y_0}

(defn create-segment [{:keys [start end]}]
...
(defn horizontal? [{:keys [start end]}]
  (= (:x start) (:x end)))

(defn vertical? [{:keys [start end]}]
  (= (:y start) (:y end)))

(defn diagonal? [{:keys [start end]}]
    (let [dx (- (:x end) (:x start))
          dy (- (:y end) (:y start))]
      (= (abs dx) (abs dy))))

(defn eval-solution [input & preds]
  (->> input
       (parse-input)
...
(assert (= 5
           (eval-solution example-input vertical? horizontal?)))

(assert (= 6548
           (eval-solution input vertical? horizontal?)))

(assert (= 12
           (eval-solution example-input vertical? horizontal? diagonal?)))

(assert (= 19663
           (eval-solution input vertical? horizontal? diagonal?)))
