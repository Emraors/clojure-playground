(ns metacircular-evaluator.expression-test
  (:require
   ...
   [metacircular-evaluator.expression :refer :all]))


(deftest self-evaluating?-test
  (testing "self-evaluating"
    (are [expected actual] (= expected (self-evaluating? actual))
      true  1
      true  1.0
      true  "a"
      nil   true
...
(deftest variable?-test
  (testing "variable"
    (are [expected actual] (= expected (variable? actual))
      true 'x
      true 'y
      false  1
      false :a)))
...
(deftest cond?-test
  (testing "cond"
    (is (= true (cond? '(cond (true? 1) 1 (false? 2) 2))))
